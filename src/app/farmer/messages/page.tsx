...
export default function FarmerMessagesPage() {
  ...
  // Sync selection with initial url parameter or active conversation
  useEffect(() => {
    if (initialConvId) {
  ...
  // stable height = viewport - navbar(~68px) - layout padding(~80px)
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 148px)' }}>
      {/* ─── Dual Pane Layout — no layout shift, both panes always in DOM ── */}
          ...
          {selectedConv ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-white border border-[#E5E7EB] rounded-2xl p-8 text-center shadow-xs">
              ...
              <p className="text-xs text-[#6B7280] max-w-sm mt-1">
                Choose a consultation from the list on the left to view message history and chat in real-time.
              </p>
            </div>
          )}
        </div>
      ...
      {/* ─── Mobile Back Button — outside grid so it never shifts pane heights */}
      {mobileShowChat && (
