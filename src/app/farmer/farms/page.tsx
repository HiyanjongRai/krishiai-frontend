...
export default function FarmerFarmsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-[#E5E7EB]/80 shadow-2xs">
        <div className="flex-1 min-w-[240px]">
          <input
            ...
            className="w-full text-xs sm:text-sm px-3.5 py-2 rounded-xl bg-[#F8FAF8] border border-[#E5E7EB] focus:bg-white focus:border-[#2E7D32] focus:outline-none transition-colors"
          />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
          <select
            ...
            className="text-xs font-semibold px-3 py-2 rounded-xl bg-[#F8FAF8] border border-[#E5E7EB] text-[#4B5563] focus:outline-none focus:border-[#2E7D32]"
          >
      ...
      {/* Error Banner */}
      {error && (
        <div className="flex items-center justify-between p-4 rounded-2xl bg-[#FEE2E2] border border-[#FCA5A5] text-[#DC2626] text-xs">
          <div className="flex items-center gap-2">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-3xl border border-[#E5E7EB] bg-white p-6 space-y-4 animate-pulse">
              <div className="h-6 bg-[#E5E7EB] rounded-md w-3/4" />
              <div className="h-4 bg-[#F1F5F2] rounded-md w-1/2" />
              <div className="h-24 bg-[#F8FAF8] rounded-xl" />
            </div>
          ))}
        </div>
      ) : filteredFarms.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-[#D1D5DB] bg-white p-12 text-center space-y-4">
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-lg font-black text-[#1F2937] tracking-tight group-hover:text-[#2E7D32] transition-colors">
                        {farm.farmName}
                      </h3>
                      {index === 0 && (
                        <span className="px-2 py-0.5 rounded-full bg-[#E8F5E9] text-[#1B5E20] text-[10px] font-extrabold uppercase tracking-wider">
                          Primary
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 text-xs text-[#6B7280] font-medium mt-0.5">
                      ...
                      {farm.location?.type && (
                        <span className="text-[10px] text-[#9CA3AF]">({farm.location.type})</span>
                      )}
                    </div>
                  </div>

                  <span className="px-2.5 py-1 rounded-full bg-[#F1F5F2] text-[#4B5563] text-[10px] font-bold uppercase tracking-wider border border-[#E5E7EB] shrink-0">
                    {farm.farmType?.replace("_", " ") || "FARM"}
                  </span>
                </div>

                {/* Area & Coordinates info */}
                <div className="grid grid-cols-2 gap-2 my-4 p-3 rounded-2xl bg-[#F8FAF8]/80 border border-[#E5E7EB] text-xs">
                  <div>
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-[#9CA3AF]">Land Area</span>
                    <p className="font-bold text-[#1F2937] mt-0.5">
                      {farm.area ? `${farm.area} ${farm.areaUnit?.toLowerCase() || ""}` : "Unspecified"}
                    </p>
                  </div>
                  <div>
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-[#9CA3AF]">Coordinates</span>
                    <p className="font-bold text-[#1F2937] mt-0.5">
                      {farm.latitude && farm.longitude ? (
                        ...
                        <span className="text-[#9CA3AF] font-normal">Not configured</span>
                      )}
                    </p>
                  </div>
                </div>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#E5E7EB] space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
              <div>
                <h3 className="text-lg font-black text-[#1F2937]">Register New Farm</h3>
                <p className="text-xs text-[#6B7280]">Record a plot to link crop schedules and diagnosis.</p>
              </div>
              <button
                onClick={() => setIsCreateOpen(false)}
                className="p-2 rounded-xl text-[#9CA3AF] hover:text-[#4B5563] hover:bg-[#F1F5F2]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#4B5563] mb-1.5">Region / Location</label>
                  <select
                    value={formLocationId}
                    onChange={(e) => setFormLocationId(e.target.value)}
                    className="w-full text-xs font-semibold px-3 py-2.5 rounded-xl border border-[#E5E7EB] bg-white focus:border-[#2E7D32] focus:outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-[#4B5563] mb-1.5">Farm Type</label>
                  <select
                    value={formFarmType}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-[#4B5563] mb-1.5">Area Unit</label>
                  <select
                    value={formAreaUnit}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-bold text-[#4B5563]">GPS Coordinates (Optional)</label>
                  <button
                    type="button"
                    onClick={handleUseCurrentLocation}
                    className="text-[11px] font-bold text-[#2E7D32] hover:underline inline-flex items-center gap-1"
                  >
              ...
              <div>
                <label className="block text-xs font-bold text-[#4B5563] mb-1.5">Field Notes / Description</label>
                <textarea
                  rows={3}
                  placeholder="Irrigation sources, soil texture notes, terrace layout..."
                  value={formDescription}
                  onChange={(e) => setFormDescription(e.target.value)}
                  className="w-full text-xs p-3 rounded-xl border border-[#E5E7EB] focus:border-[#2E7D32] focus:outline-none"
                />
              </div>
      ...
      {editingFarm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#E5E7EB] space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
              <div>
                <h3 className="text-lg font-black text-[#1F2937]">Edit Farm Information</h3>
                <p className="text-xs text-[#6B7280]">Update parcel coordinates and classification.</p>
              </div>
              <button
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-bold text-[#4B5563]">GPS Coordinates</label>
                  <button
                    type="button"
                    onClick={handleUseCurrentLocation}
                    className="text-[11px] font-bold text-[#2E7D32] hover:underline inline-flex items-center gap-1"
              ...
              <div>
                <label className="block text-xs font-bold text-[#4B5563] mb-1.5">Field Notes</label>
                <textarea
                  rows={3}
                  value={formDescription}
                  onChange={(e) => setFormDescription(e.target.value)}
      ...
      {deletingFarmId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl border border-[#E5E7EB] space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-[#FEE2E2] text-[#DC2626] flex items-center justify-center">
                <Trash2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-bold text-[#1F2937]">Remove Farm Parcel?</h3>
                <p className="text-xs text-[#6B7280]">This action can be restored later.</p>
              </div>
            </div>
