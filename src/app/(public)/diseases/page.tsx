  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 py-16 space-y-8">
        <h1 className="text-3xl font-extrabold text-[#1F2937]">Crop Disease Knowledge Base</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sampleDiseases.map((d) => (
            <Card key={d.id} className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-[#1F2937]">{d.name}</h3>
                <Badge variant={d.severity === "HIGH" ? "danger" : "warning"}>{d.severity} Severity</Badge>
              </div>
              <p className="text-xs italic text-[#6B7280]">{d.scientificName}</p>
              <p className="text-xs text-[#4B5563] font-semibold">Symptoms:</p>
